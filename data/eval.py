...
def main(args):
    ...
    return calculate(args, dataset_inc_exc, results)


def calculate(args, dataset_inc_exc, results):
    ...
    dataset_inc_exc[args.label_field_name] = dataset_inc_exc[args.label_field_name].transform(
    ...
    if args.rogue_tokens:
        inc_exc['prediction'] = inc_exc[args.response_field_name].fillna("").apply(
            lambda x: next(re.finditer(r'(include)|(exclude)', x.lower()), ["error"])[0])
        inc_exc['prediction'] = inc_exc['prediction'].transform(lambda x: label_mapping[x] if x in label_mapping else x)
    else:
        inc_exc['prediction'] = inc_exc[args.response_field_name].str.split().str[0]
    merged = pd.merge(dataset_inc_exc, inc_exc, on=['instruction', 'input'], how='left')
    merged = merged.dropna(subset=['prediction'])
    try:
        class_report = metrics.classification_report(merged[args.label_field_name], merged['prediction'], digits=2,
                                                     labels=['Included', 'Excluded'], output_dict=True)
        conf_mat = metrics.confusion_matrix(merged[args.label_field_name], merged['prediction'],
                                            labels=['Included', 'Excluded'])
    except:
        class_report = metrics.classification_report(merged[args.label_field_name + '_x'], merged['prediction'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--response_field_name', default='response', type=str, help='Path to save results')
